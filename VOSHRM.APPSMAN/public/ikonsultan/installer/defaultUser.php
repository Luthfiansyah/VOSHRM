...
function submitDefUserInfo() {
	...
	if(frm.OHRMAdminUserName.value.length < 5) {
		alert('HRM Admin User-name should be at least 5 char. long!');
		frm.OHRMAdminUserName.focus();
		return;
	}

	if(frm.OHRMAdminPassword.value == '') {
		alert('HRM Admin Password left Empty!');
		frm.OHRMAdminPassword.focus();
		return;
	}

	if(frm.OHRMAdminPassword.value != frm.OHRMAdminPasswordConfirm.value) {
		alert('HRM Admin Password and Confirm HRM Admin Password don\'t match!');
		frm.OHRMAdminPassword.focus();
		return;
	}
...
<div id="content">
	<h2>Step 4: Admin User Creation</h2>
        ...
        <p>After HRM is configured you will need an Administrator Account to Login into HRM. <br />
